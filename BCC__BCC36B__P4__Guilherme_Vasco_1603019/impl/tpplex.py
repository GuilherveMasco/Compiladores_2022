...
# impressão do token comentário (comentar return para não imprimir)
t_COMENTARIO = r'(\{((.|\n)*?)\})'
...
def main():
    ...
    while True:
      ...
      if not tok:
        break      # No more input
      #print(tok) -> para impressão do token com informação de posição
      print(tok.type) # impressao do token (tok.type)
      #print(tok.value + ":" + tok.type) -> para token do tipo (valor:token)
